    ...
    const countResult =
      roleToUse === "teacher"
        ? await db
            ...
            .leftJoin(subjects, eq(classes.subjectId, subjects.id))
            .where(
              and(eq(user.role, roleToUse), eq(subjects.departmentId, departmentId))
            )
        : await db
            .select({ count: sql<number>`count(distinct ${user.id})` })
            .from(user)
